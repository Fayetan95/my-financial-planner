...
A demo-first AI financial & retirement planner that runs a personalized projection engine, captures leads, and shows an interactive dashboard — deployable in a week as a polished portfolio piece.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "26497130+Fayetan95@users.noreply.github.com"
  git config user.name "Fayetan95"
  ```
